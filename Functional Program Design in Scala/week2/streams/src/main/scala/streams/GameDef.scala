...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    //      def trace(directions: List[(Block, Move)]): List[(Block, Move)] = {
    //        for (direction <- directions if direction._1.isLegal) yield direction :: trace(direction._1.neighbors)
    //        Nil
    //      }

    //      def trace(nextNeighbors: List[(Block, Move)], acc: List[(Block, Move)]): List[(Block, Move)] = {
    //        nextNeighbors.filter(_._1.isLegal).foreach { legalNeighbor =>
    //          println(legalNeighbor)
    //          trace(legalNeighbor._1.neighbors, legalNeighbor :: acc)
    //        }
    //        acc
    //      }
    //
