...
class KMeans {
  ...
  /**
    * Update the mean values to the average of all the points in the cluster.
    */
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    var i = 0
    val u = oldMeans.size
    var stillConverged = true
    while (stillConverged & i < u) {
      stillConverged &= oldMeans(i).squareDistance(newMeans(i)) <= eta
      i += 1
    }
    stillConverged
  }
