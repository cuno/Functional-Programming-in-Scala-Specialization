...
package object barneshut {
  ...
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    ...
    def insert(b: Body): Fork =
      if (b.x > nw.centerX)
        if (b.y > ne.centerY) Fork(nw, ne, sw, se.insert(b)) else Fork(nw, ne.insert(b), sw, se)
      else if (b.y > nw.centerY) Fork(nw, ne, sw.insert(b), se) else Fork(nw.insert(b), ne, sw, se)
  }

  /*
   * If the size of a Leaf is greater than a predefined constant minimumSize, inserting an additonal body into that
   * Leaf quadtree creates a Fork quadtree with empty children, and adds all the bodies into that Fork (including the
   * new body). Otherwise, inserting creates another Leaf with all the existing bodies and the new one.
   *
   * NW NE
   * SW SE
   */
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    extends Quad {
    val mass = bodies.foldLeft(0f)((acc, b) => acc + b.mass)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    val sectorSizeInt = sectorSize.toInt
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      val bucket = apply(if (x < sectorPrecision) x else sectorPrecision - 1, if (y < sectorPrecision) y else sectorPrecision - 1)
      bucket += b
      this
    }
